...
export function VSLAMembersStats({ members }: VSLAMembersStatsProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card *:data-[slot=card]:shadow-xs md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 dark:*:data-[slot=card]:bg-card">
      <MetricCard
        title="Total Members"
        value={totalMembers}
        footer={{
